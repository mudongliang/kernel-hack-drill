...
# create-image.sh creates a minimal Debian Wheezy image suitable for syzkaller.
...
DEBOOTSTRAP_PARAMS="--include=$PREINSTALL_PKGS --components=main,contrib,non-free $RELEASE $DIR http://archive.debian.org/debian/"
if [ $ARCH != $(uname -m) ]; then
...
# Set to the machine for drill
sudo mkdir -p $DIR/drill/.ssh/
cat $RELEASE.id_rsa.pub | sudo tee $DIR/drill/.ssh/authorized_keys
